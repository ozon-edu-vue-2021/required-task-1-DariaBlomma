...
const renderPictures = function(list) {
  ...
  // fragment - элемент со всеми картинками
  const fragment = document.createDocumentFragment();

  list.forEach(function(element) {
    // clone - отдельный элемент с картинкой, при вставке во fragment копия очищается
...
const imageHandler = function(evt) {
  ...
  if (evt.target.closest("a")) {
    // evt.target в большинстве случаев ведет на img, a data-id есть у родителя <a ..></a>
    getPictureInfo(evt.target.closest("a").dataset.id);
  }
};

action.addEventListener("click", actionHandler);
// container -секция с картинками
